describe('FormulaUpdateObserver', function() {
  ...
  it('should fire `update` hook on every modified formula', function() {
    ...
    // add 'gt'
    expect(updateSpy.calls.argsFor(0)[0].column).toBe(0);
    expect(updateSpy.calls.argsFor(0)[0].formulas.length).toBe(1);
    expect(updateSpy.calls.argsFor(0)[0].formulas[0].name).toBe('gt');
    expect(updateSpy.calls.argsFor(0)[0].formulas[0].args).toEqual([2]);
    // remove
    expect(updateSpy.calls.argsFor(1)[0].column).toBe(2);
    expect(updateSpy.calls.argsFor(1)[0].formulas.length).toBe(0);
    // add 'contains'
    expect(updateSpy.calls.argsFor(2)[0].column).toBe(1);
    expect(updateSpy.calls.argsFor(2)[0].formulas.length).toBe(1);
    expect(updateSpy.calls.argsFor(2)[0].formulas[0].name).toBe('contains');
    expect(updateSpy.calls.argsFor(2)[0].formulas[0].args).toEqual(['b']);
    // add 'begins_with'
    expect(updateSpy.calls.argsFor(3)[0].column).toBe(2);
    expect(updateSpy.calls.argsFor(3)[0].formulas.length).toBe(1);
    expect(updateSpy.calls.argsFor(3)[0].formulas[0].name).toBe('begins_with');
    expect(updateSpy.calls.argsFor(3)[0].formulas[0].args).toEqual(['c']);
    // add 'ends_with'
    expect(updateSpy.calls.argsFor(4)[0].column).toBe(2);
    expect(updateSpy.calls.argsFor(4)[0].formulas.length).toBe(2);
    expect(updateSpy.calls.argsFor(4)[0].formulas[1].name).toBe('ends_with');
    expect(updateSpy.calls.argsFor(4)[0].formulas[1].args).toEqual(['d']);
    // clean
    expect(updateSpy.calls.argsFor(5)[0].column).toBe(0);
    expect(updateSpy.calls.argsFor(6)[0].column).toBe(1);
    expect(updateSpy.calls.argsFor(7)[0].column).toBe(2);
  });
  ...
  describe('groupChanges', function() {
    it('should fire `update` hook only once on `flush` method call when groupChanges is enabled', function() {
      ...
      expect(updateSpy.calls.argsFor(1)[0].column).toBe(2);
      expect(updateSpy.calls.argsFor(1)[0].formulas.length).toBe(2);
      expect(updateSpy.calls.argsFor(1)[0].formulas[0].name).toBe('begins_with');
      expect(updateSpy.calls.argsFor(1)[0].formulas[0].args).toEqual(['c']);
      expect(updateSpy.calls.argsFor(1)[0].formulas[1].name).toBe('ends_with');
      expect(updateSpy.calls.argsFor(1)[0].formulas[1].args).toEqual(['d']);
      // add 'contains'
      expect(updateSpy.calls.argsFor(2)[0].column).toBe(1);
      expect(updateSpy.calls.argsFor(2)[0].formulas.length).toBe(1);
      expect(updateSpy.calls.argsFor(2)[0].formulas[0].name).toBe('contains');
